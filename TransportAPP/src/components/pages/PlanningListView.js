...
const PlanningListView = () => {
  ...
  const cleanupDeletedWeeks = () => {
    try {
      ...
      if (hasChanges) {
        ...
        console.log('🧹 Nettoyage des anciennes entrées supprimées effectué dans PlanningListView');
      }
    } catch (error) {
  ...
  const handleExportPlanningCSV = async (planning) => {
    try {
      // Préparer les données pour l'export CSV
      const csvData = [];
      const DAYS = ['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche'];
      let dayAssignments = planning.day_assignments ? (typeof planning.day_assignments === 'string' ? JSON.parse(planning.day_assignments) : planning.day_assignments) : {};
      const teamKeys = ['Matin','Soir','Nuit','Normal'];
      // Fallback: si vide, charger depuis l'API détaillée
      if (!dayAssignments || Object.keys(dayAssignments).length === 0) {
        try {
          const token = localStorage.getItem('authToken');
          const resp = await fetch(`http://localhost:3001/api/weekly-plannings/${planning.year}/${planning.week_number}`, {
            credentials: 'include',
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (resp.ok) {
            const detail = await resp.json();
            dayAssignments = detail.day_assignments ? (typeof detail.day_assignments === 'string' ? JSON.parse(detail.day_assignments) : detail.day_assignments) : {};
          }
        } catch (e) {
          console.warn('⚠️ Impossible de recharger day_assignments:', e);
        }
      }
      ...
      if (planning.enriched_assignments && Object.keys(planning.enriched_assignments).length > 0) {
        ...
        Object.values(planning.enriched_assignments).forEach((employee) => {
          const idNum = parseInt(employee.id, 10);
          const TEAM_KEYS_LOCAL = ['Matin','Soir','Nuit','Normal'];
          TEAM_KEYS_LOCAL.forEach(team => {
            ...
            const dayFlags = DAYS.map(day => {
              const arr = Array.isArray((dayAssignments?.[day] || {})[team]) ? (dayAssignments?.[day] || {})[team] : [];
              const checked = arr.some(v => parseInt(v, 10) === idNum);
              return checked ? '✔' : '';
            });
  ...
  const loadPlannings = async () => {
    try {
      ...
      const filteredPlannings = weeklyPlannings.filter(planning => {
        const weekKey = `${planning.year}-W${planning.week_number}`;
      ...
      console.log(`📊 ${weeklyPlannings.length} plannings récupérés, ${filteredPlannings.length} après filtrage des suppressions`);
      ...
      const transformedPlannings = filteredPlannings.map(planning => {
        ...
        Object.entries(assignmentsObj).forEach(([equipe, employeeIds]) => {
          ...
          employeeIds.forEach(raw => {
            const empId = (raw && typeof raw === 'object') ? parseInt(raw.id, 10) : parseInt(raw, 10);
            if (!Number.isFinite(empId)) return;
            const employee = users.find(u => u.id === empId);
            if (!employee) return;
            if (!enrichedAssignments[String(empId)]) {
              enrichedAssignments[String(empId)] = {
                id: empId,
                equipes: [],
                nom: employee.nom,
            ...
            if (!enrichedAssignments[String(empId)].equipes.includes(equipe)) {
              enrichedAssignments[String(empId)].equipes.push(equipe);
            }
          });
        });
        ...
        return {
          ...
          day_assignments: planning.day_assignments, // pour export jours
  ...
  const reopenPlanning = async (planning) => {
    try {
      ...
      if (planning.raw_status === 'completed') {
        // Approve reopen directly (admin/RH)
        ...
        if (!resp.ok) {
          ...
          return alert(`Erreur réouverture: ${t}`);
        }
      ...
      alert('Erreur réseau réouverture');
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">{formatDate(selectedPlanning.date_debut)}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Date Fin
                    </label>
                    <p className="text-gray-900">{formatDate(selectedPlanning.date_fin)}</p>
                  </div>
                </div>
                    ...
                    <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg">
                      ...
                      <div className="divide-y divide-gray-200">
                        {Object.values(selectedPlanning.enriched_assignments).map((employee, index) => {
                          ...
                          // Générer une ligne par équipe où l'employé est présent au moins un jour
                          const teamRows = TEAM_KEYS.map(team => {
                            const hasAny = DAYS.some(day => {
